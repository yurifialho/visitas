...
#-------------------------------------------------------------------------- 
	...
	if($action == "index") {
		...
		if($ano != NULL && isNumeric($ano)){
			...
			$router->addMsgErro("Ano informado invбlido.");
		}
		if($mes != NULL && isNumeric($mes)){
			$router->addParam("mes", $mes);
		} else {
			$router->addMsgErro("Mкs informado invбlido.");
		}
		$router->redirect();
		return;
	} elseif($action == "agendar") {
		if(!validateRecaptch()) {
			$router->addMsgErro("Cуdigo nгo confere com a imagem!");
		...
		if($dispo != null && $dispo->reserva == null) {
			$router->addMsgErro("Essa data jб encontra-se reservada por outra pessoa, por favor tente agendar em outra data!");
			$router->redirect(); return;
		}
		...
		if($reserva->save()){
			$router->addMsg("Objeto salvo com sucesso! A reserva encontra-se em anбlise para aprovaзгo.");
		...
		$router->addMsgErro("Operaзгo nгo suportada!");
		$router->redirect(); return;
	}  		
?>