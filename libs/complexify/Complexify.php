...
class Complexify
{
    ...
    private static $BANLIST = array('0','1111','1212','1234','1313','2000','2112','2222',
        ...
        'password1','password12','password123','brasil','patria');
    ...
    public function evaluateSecurity($password)
    {
        ...
        if (!$this->inBanlist($password)) {
            ...
            array_push($error, 'muito comum (Nao coloque uma palavra do cotidiano)');
            $complexity = 1;
        }
        ...
        if ($complexity <= self::$MIN_COMPLEXITY) {
            array_push($error, 'muito simples (Coloque Alfanumerico, Caracteres especiais e numeros)' . self::$MIN_COMPLEXITY);
        }
        if (mb_strlen($password, $this->encoding) < $this->minimumChars) {
            array_push($error, 'muito curta (Minimo: ' . $this->minimumChars . ' caracteres)');
        }
